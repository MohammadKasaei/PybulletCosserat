from pybullet_env.BasicEnvironment_ThreeSegments import SoftRobotBasicEnvironment_ThreeSegments
import numpy as np
import time
...
if __name__ == '__main__':
    ...
    env = SoftRobotBasicEnvironment_ThreeSegments(moving_base=False)
    ...
    sign = 1   
    env._set_marker(np.array([0.1,0.,0.1]),radius=0.02,color=[0,1,0,1])
    ...
    while True:    
        ...
        # for i in range(200):
            ...
            env.move_robot(np.array([0.0,sign*(400-i)/10000.0,0.0, 0, -sign*(400-i)/10000.0,0.0]))
            time.sleep(0.01)
            env._dummy_sim_step(100)
            # if i % 10 == 0 :
            #     env.in_hand_camera_capture_image()    
