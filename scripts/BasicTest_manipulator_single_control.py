...
def Jac(f, q, dq=np.array((1e-4,1e-4,1e-4,1e-4,1e-4,1e-4,1e-4,1e-4,1e-4))):
    ...
    for j in range(n):  # through rows 
        if (j==0):
            ...
            jac [j,:] = 0 #(f(q[0]+Dq,q[1],q[2])[0] - f(q[0]-Dq,q[1],q[2])[0])/dq[j]
        elif (j==1):
...
if __name__ == "__main__":
    ...
    shape, ode_sol = soft_robot_1.move_robot_ori(action=sf_action, base_pos = new_pos, base_orin = base_orin)
    xc = shape[-1][:3]
    x0 = np.copy(xc)
    ...
    for i in range(int(tf/ts)):
        ...
        sf_action = 0.95*sf_action + 0.05*(qdot[:6] * ts)
        sf_action[0] = 0
        sf_action[3] = 0
        sf_action = sf_action.clip(-0.015,0.015)
